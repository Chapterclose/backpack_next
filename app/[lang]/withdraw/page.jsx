        ...
        <ul>
          {currenciesListArr?.map((item) => (
            <Link
              prefetch
              key={item.id}
              href={`/en/withdraw-apply?coin=${item.title.toLocaleLowerCase()}`}
              className="flex items-center justify-between hover:bg-gray-100 dark:hover:bg-gray-800 px-3 rounded"
            >
