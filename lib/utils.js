...
const baseURL = "http://45.55.211.187/api";
...
export const AmountWithCommas = (amount, coin) => {
  if (amount === null || amount === undefined || amount === "") {
    ...
    return lowerCoin === "eth" || lowerCoin === "btc" ? "0.0000000" : "0.00";
  }
  ...
  const lowerCoin = coin?.toLowerCase();
  const decimalPlaces =
    lowerCoin === "usdt" ? 2 : lowerCoin === "eth" || lowerCoin === "btc" ? 7 : 2;

  // ✅ Convert number to full decimal string (avoid 8e-8 issue)
  let str = num.toLocaleString("en-US", { useGrouping: false, maximumFractionDigits: 20 });
