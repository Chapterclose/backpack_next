...
const MobileNav = () => {
  ...
  const navItems = [
    ...
    { href: "/en/trade?symbol=btc", icon: Activity, label: "Trade", dynamicPath: "/en/trade" }, // Add a dynamicPath property
    ...
    <div className="lg:hidden shadow dark:shadow-2xl">
      ...
      <div className="fixed bottom-0 bg-gray-900 w-full grid grid-cols-4 gap-x-2 z-50">
        {navItems.slice(0, 4).map((item) => {
          ...
          // Modified isActive check for dynamic paths
      ...
      >
        ...
        <div className="px-6 mb-3">
          <h4 className="text-lg font-medium">
            Email: <span className="text-base">{walletAddress}</span>
          </h4>
        ...
        <div className="overflow-y-auto h-full">
          <ul className="flex flex-col px-6 pt-6 space-y-4">
            {navItems.slice(4).map((item) => {
              const Icon = item.icon;
              // Modified isActive check for dynamic paths in sidebar
              const isActive = item.dynamicPath
                ? pathname.startsWith(item.dynamicPath)
                : pathname === item.href;
              return (
                <li key={item.label}>
                  <button
                    ...
                    className={twMerge(
                      ...
                      isActive // Use the new isActive for sidebar items too
                        ? "text-yellow-400"
          ...
          >
            <LogOut className="group-hover:text-primary duration-300 cursor-pointer" />{" "}
            <button className="font-semibold group-hover:text-primary duration-300 cursor-pointer pl-2">
              Log Out
            </button>
          </div>
        </div>
