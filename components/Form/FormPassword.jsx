...
function FormPassword({label, type, placeholder,className, onChange, value, ...props}) {
    const [isEye, setIsEye] = useState(false)
    return ( 
        <div>
            {label !== "" && <label className="text-lg mb-2 dark:text-white block font-medium">{label}</label>}
            <div className="relative">
                <input onChange={onChange} type={isEye ? "text" :"password"} placeholder={placeholder} value={value} className={twMerge(
          ...
          "dark:bg-gray-800 dark:text-white dark:placeholder-white")} />
                <Eye onClick={()=>setIsEye(!isEye)} className="absolute right-2 cursor-pointer top-1/2 translate-y-[-50%] dark:text-white"/>
            </div>
        </div>
     );
}
